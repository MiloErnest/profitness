...
@section('title', 'Reporte de Cafetería - Pro.Fitness')
...
<div class="min-h-screen bg-gray-100 py-8">
    <div class="container mx-auto px-4">
            ...
            <div class="flex justify-between items-center">
                <div>
                    <h2 class="text-3xl font-bold text-gray-800">
                        <i class="fas fa-chart-bar text-orange-600 mr-2"></i>Reporte Detallado de Cafetería
                    </h2>
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('cafeteria.admin') }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg transition duration-300">
        ...
        <!-- Selector de Período -->
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
            <form action="{{ route('cafeteria.report') }}" method="GET" class="flex flex-wrap gap-4 items-end">
                <div class="flex-1 min-w-[150px]">
                    <label class="block text-gray-700 font-semibold mb-2">
                        <i class="fas fa-calendar-alt mr-1 text-orange-600"></i>Mes
                    </label>
                    <select name="month" 
                ...
                <div class="flex-1 min-w-[150px]">
                    <label class="block text-gray-700 font-semibold mb-2">
                        <i class="fas fa-calendar mr-1 text-orange-600"></i>Año
                    </label>
                    <input type="number" 
                           name="year" 
                           value="{{ $currentYear }}"
                ...
                <div>
                    <a href="{{ route('cafeteria.currentMonth') }}" class="bg-teal-600 hover:bg-teal-700 text-white px-6 py-3 rounded-lg transition duration-300 font-semibold inline-block">
                ...
                @else
                    <div>
                        <button type="button" 
                                ...
                                class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg transition duration-300 font-semibold">
            ...
            @if(!$isClosed)
                <form id="closeMonthForm" action="{{ route('cafeteria.closeMonth') }}" method="POST" class="hidden">
                    @csrf
            ...
            @if($isClosed)
                <form id="updateMonthForm" action="{{ route('cafeteria.updateMonth') }}" method="POST" class="hidden">
                    @csrf
                    <input type="hidden" name="year" value="{{ $currentYear }}">
                    <input type="hidden" name="month" value="{{ $currentMonth }}">
                </form>
                            ...
                            <div class="mb-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                    @foreach($months as $period)
                                        <div class="bg-white border-2 {{ $currentYear == $period['year'] && $currentMonth == $period['month'] ? 'border-blue-500 bg-blue-50' : 'border-gray-200' }} rounded-lg p-4 hover:shadow-md transition duration-200">
                                            <div class="flex items-center justify-between mb-2">
                                                <a href="{{ route('cafeteria.report', ['year' => $period['year'], 'month' => $period['month']]) }}" 
                                                   class="flex-1 font-semibold text-gray-800 hover:text-blue-600">
                                                    <i class="fas fa-file-alt mr-2 text-blue-500"></i>{{ $period['label'] }}
                                                </a>
                                                <form action="{{ route('cafeteria.deleteMonth') }}" 
                                                      method="POST" 
                                                      class="inline"
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    <thead>
                        <tr class="bg-orange-200 border-b-2 border-orange-400">
                        ...
                        <tr class="bg-gray-200">
                            <th class="px-4 py-3 text-left font-semibold text-gray-700 border border-gray-300">MERCANCÍAS</th>
                            <th class="px-4 py-3 text-center font-semibold text-gray-700 border border-gray-300 bg-blue-100">RECIBIDO</th>
                            <th class="px-4 py-3 text-center font-semibold text-gray-700 border border-gray-300 bg-yellow-100">MERCANCÍA<br>ADICIONAL</th>
                            <th class="px-4 py-3 text-center font-semibold text-gray-700 border border-gray-300 bg-green-100">TOTAL<br>MERCANCÍA</th>
                            <th class="px-4 py-3 text-center font-semibold text-gray-700 border border-gray-300 bg-red-100">SALIDA<br>(Vendido)</th>
                            ...
                            <th class="px-4 py-3 text-center font-semibold text-gray-700 border border-gray-300 bg-purple-100">VALOR</th>
                            <th class="px-4 py-3 text-center font-semibold text-gray-700 border border-gray-300 bg-blue-200">TOTAL<br>VALOR</th>
                            <th class="px-4 py-3 text-center font-semibold text-gray-700 border border-gray-300 bg-indigo-100">BASE<br>(Costo)</th>
                        </tr>
                    </thead>
                    <tbody>
                        @php
                        ...
                        @forelse($dataToShow as $product)
                            @php
                                ...
                                if ($isClosed && $savedReport->isNotEmpty()) {
                                    $recibido = $product->received ?? 0;
                                    $adicional = $product->additional_stock ?? 0;
                                    $vendido = $product->sold ?? 0;
                                ...
                                $totalMercanciaProducto = $recibido + $adicional;
                                $totalSalidasProducto = $vendido + $perdidas;
                                $valorTotalProducto = $stock * $precio;
                                $costoTotalProducto = $stock * $costPrice;
                                
                                // Acumular totales
                                $totalRecibido += $recibido;
                                $totalAdicional += $adicional;
                                $totalMercancia += $totalMercanciaProducto;
                                $totalSalida += $vendido;
                                $totalPerdidas += $perdidas;
                                $totalStock += $stock;
                                $totalValorInventario += $valorTotalProducto;
                                $totalCosto += $costoTotalProducto;
                            @endphp
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3 border border-gray-300 font-semibold">{{ $product->product_name }}</td>
                                <td class="px-4 py-3 border border-gray-300 text-center bg-blue-50">{{ $recibido }}</td>
                                <td class="px-4 py-3 border border-gray-300 text-center bg-yellow-50">{{ $adicional }}</td>
                                <td class="px-4 py-3 border border-gray-300 text-center bg-green-50 font-semibold">{{ $totalMercanciaProducto }}</td>
                                <td class="px-4 py-3 border border-gray-300 text-center bg-red-50">{{ $vendido }}</td>
                                <td class="px-4 py-3 border border-gray-300 text-center bg-orange-50">{{ $perdidas }}</td>
                                <td class="px-4 py-3 border border-gray-300 text-center bg-green-100 font-bold">{{ $stock }}</td>
                                <td class="px-4 py-3 border border-gray-300 text-center bg-purple-50">${{ number_format($precio, 0, ',', '.') }}</td>
                                <td class="px-4 py-3 border border-gray-300 text-right bg-blue-50 font-semibold">${{ number_format($valorTotalProducto, 0, ',', '.') }}</td>
                                <td class="px-4 py-3 border border-gray-300 text-right bg-indigo-50">${{ number_format($costoTotalProducto, 0, ',', '.') }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="10" class="px-4 py-8 text-center text-gray-500">
                                    <i class="fas fa-inbox text-4xl mb-2"></i>
                                    <p>No hay productos para mostrar en este período</p>
                                </td>
                            </tr>
                        @endforelse
                        ...
                        @if($products->isNotEmpty())
                            <tr class="bg-gray-700 text-white font-bold">
                                <td class="px-4 py-3 border border-gray-600">TOTALES</td>
                                ...
                                <td class="px-4 py-3 border border-gray-600 text-center">{{ $totalSalida }}</td>
                                ...
                                <td class="px-4 py-3 border border-gray-600 text-right">${{ number_format($totalValorInventario, 0, ',', '.') }}</td>
                                <td class="px-4 py-3 border border-gray-600 text-right">${{ number_format($totalCosto, 0, ',', '.') }}</td>
                            </tr>
                        @endif
                    </tbody>
                </table>
        ...
        <div class="mt-6 bg-gradient-to-r from-orange-50 to-yellow-50 rounded-lg shadow-md p-6 border-l-4 border-orange-600">
            <h3 class="font-bold text-gray-800 mb-4 text-xl">
                <i class="fas fa-chart-line text-orange-600 mr-2"></i>Resumen Financiero
            </h3>
        ...
        <div class="mt-6 bg-white rounded-lg shadow-md p-6">
            <h3 class="font-bold text-gray-800 mb-3">
                <i class="fas fa-info-circle text-orange-600 mr-2"></i>Información del Reporte
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
                <div>
                    <p><strong>Total Mercancía:</strong> Recibido + Adicional</p>
                    <p><strong>Stock:</strong> Total Mercancía - (Vendido + Pérdidas)</p>
                </div>
                <div>
                    <p><strong>Total Valor:</strong> Stock × Precio de Venta</p>
                    <p><strong>Base (Costo):</strong> Stock × Precio de Compra</p>
                </div>
            </div>
        </div>
    </div>
