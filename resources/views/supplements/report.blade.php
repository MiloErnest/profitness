...
@section('title', 'Reporte de Suplementos - Pro.Fitness')
...
<div class="min-h-screen bg-gray-100 py-8">
    <div class="container mx-auto px-4">
            ...
            <div class="flex justify-between items-center">
                <div>
                    <h2 class="text-3xl font-bold text-gray-800">
                        <i class="fas fa-chart-bar text-blue-600 mr-2"></i>Reporte Detallado de Inventario
                    </h2>
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('supplements.admin') }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg transition duration-300">
        ...
        <!-- Selector de Período -->
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
            <form action="{{ route('supplements.report') }}" method="GET" class="flex flex-wrap gap-4 items-end">
                <div class="flex-1 min-w-[150px]">
                    <label class="block text-gray-700 font-semibold mb-2">
                        <i class="fas fa-calendar-alt mr-1 text-blue-600"></i>Mes
                    </label>
                    <select name="month" 
                ...
                <div>
                    <a href="{{ route('supplements.currentMonth') }}" class="bg-teal-600 hover:bg-teal-700 text-white px-6 py-3 rounded-lg transition duration-300 font-semibold inline-block">
            ...
            @if(!$isClosed)
                <form id="closeMonthForm" action="{{ route('supplements.closeMonth') }}" method="POST" class="hidden">
                    @csrf
            ...
            @if($isClosed)
                <form id="updateMonthForm" action="{{ route('supplements.updateMonth') }}" method="POST" class="hidden">
                    @csrf
                    <input type="hidden" name="year" value="{{ $currentYear }}">
                    <input type="hidden" name="month" value="{{ $currentMonth }}">
                </form>
                ...
                <table class="min-w-full text-sm">
                    <thead>
                        <tr class="bg-orange-200 border-b-2 border-orange-400">
                            <th colspan="9" class="px-4 py-3 text-center font-bold text-gray-800">
                    ...
                    <tbody>
                        ...
                        @forelse($dataToShow as $supplement)
                            @php
                                // Si es reporte guardado, usar los campos correspondientes
                                if ($isClosed && $savedReport->isNotEmpty()) {
                                    $received = $supplement->received ?? 0;
                                    $adicional = $supplement->additional_stock ?? 0;
                                    $sold = $supplement->sold ?? 0;
                                    $perdidas = $supplement->lost_stock ?? 0;
                                    $stock = $supplement->final_stock ?? 0;
                                    $price = $supplement->sale_price ?? 0;
        ...
        @php
            ...
            foreach($dataToShow as $supp) {
                if ($isClosed && $savedReport->isNotEmpty()) {
                    $costPrice = $supp->cost_price ?? 0;
                    $salePrice = $supp->sale_price ?? 0;
                    $sold = $supp->sold ?? 0;
                    $stock = $supp->final_stock ?? 0;
                    $lost = $supp->lost_stock ?? 0;
                } else {
                    $costPrice = $supp->cost_price ?? 0;
                    $salePrice = $supp->price ?? 0;
                    $sold = $supp->sold ?? 0;
                    $stock = $supp->stock ?? 0;
                    $lost = $supp->lost_stock ?? 0;
                }
