...
@section('title', 'Editar Miembro - Pro.Fitness')
    ...
    <div class="container mx-auto px-4 max-w-3xl">
        <div class="bg-white rounded-lg shadow-md p-6 mb-6 flex justify-between items-center">
            <h2 class="text-2xl font-bold text-gray-800">
                <i class="fas fa-user-edit text-red-600 mr-2"></i>Editar miembro
            </h2>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <form action="{{ route('members.update', $member->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $member->name) }}" class="w-full border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500" required>
                    @error('name')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="email" id="email" name="email" value="{{ old('email', $member->email) }}" class="w-full border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500">
                        @error('email')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="text" id="phone" name="phone" value="{{ old('phone', $member->phone) }}" class="w-full border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500">
                        @error('phone')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <input type="date" id="start_date" name="start_date" value="{{ old('start_date', optional($member->start_date)->format('Y-m-d')) }}" class="w-full border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500" required>
                    @error('start_date')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end">
                    <button type="submit" class="bg-red-600 hover:bg-red-700 text-white px-6 py-3 rounded-lg font-semibold transition duration-300">
                        <i class="fas fa-save mr-2"></i>Actualizar miembro
                    </button>
                </div>
            </form>
        </div>
